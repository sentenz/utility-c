...
TEST(BitTest, Check)
{
  ...
  const std::vector<Tests> tests = {
    ...
    {"bit 31 in INT_MAX",   {INT_MAX, 31}, {false}},
...
TEST(BitTest, Unset)
{
  ...
  const std::vector<Tests> tests = {
    ...
    {"unset bit 0 in 0",   {0,  0}, {true, 0}},
    {"unset bit 1 in 1",   {1,  1}, {true, 1}},
...
TEST(BitTest, SetNullPointer)
{
  // Arrange
  int *x = nullptr;

  // Act
  auto got = bit_set(x, 0);

  // Assert
  EXPECT_FALSE(got);
}

TEST(BitTest, UnsetNullPointer)
{
  // Arrange
  int *x = nullptr;

  // Act
  auto got = bit_unset(x, 0);
